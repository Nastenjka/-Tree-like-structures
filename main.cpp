...
bool testBinarySearchTree()
{
    ...
    // =========================================================
    const std::string ranks[] = {"Bronze", "Silver", "Gold", "Platinum", "Diamond", "Master", "Grandmaster"};
    const int ranks_count = 7;

    for (int i = 0; i < keysAmount; i++) {
        data[i].nickname = "Player_" + std::to_string(i);
        data[i].rank = ranks[rand() % ranks_count];
        data[i].XP = (static_cast<float>(rand()) / RAND_MAX) * 10000.0f;
        data[i].total_games = (rand() % 500) + 1;
        data[i].wins = rand() % (data[i].total_games + 1);
    }
    // =========================================================
